...
module.exports = function (multichain_options, logger) {
	...
	multichain.read = function (options, cb) {
		...
		request.post({
			...
			if (body&&body.result&&body.result[indexNum]) {
				while(body.result[indexNum]){

					yValue[indexNum] = body.result[indexNum].data.hexDecode();
					var unixTimestamp = new Date(body.result[indexNum].timereceived * 1000);
					xValue[indexNum] = unixTimestamp.toLocaleString();
					lengthNum = lengthNum + 1;
					indexNum = indexNum + 1; 
			}				
			}
			return cb(null, lengthNum, xValue, yValue);
		});
	};	
	return multichain;
};